...
function idbAll(store){ return openDB().then(db=>new Promise((res,rej)=>{
...
function idbGet(store,key){ return openDB().then(db=>new Promise((res,rej)=>{
  const tx=db.transaction(store,'readonly');
  const s=tx.objectStore(store);
  const q=s.get(key);
  q.onsuccess=()=>res(q.result);
...
function idbPut(store,obj){ return openDB().then(db=>new Promise((res,rej)=>{
  try{
...
function idbDel(store,key){ return openDB().then(db=>new Promise((res,rej)=>{
  const tx=db.transaction(store,'readwrite');
  const s=tx.objectStore(store);
...
async function addChannelById(id){
  ...
  const ch = await yt('channels',{part:'snippet,statistics,contentDetails',id});
...
function sortVideos(videos, criteria){
  ...
  return videos.slice().sort((a,b)=>{
    switch(criteria){
      ...
      case 'subscribers': return (b.__ch?.subscriberCount||0)-(a.__ch?.subscriberCount||0);
      case 'latest': return new Date(b.publishedAt)-new Date(a.publishedAt);
      case 'mutantIndex': return (b.mutantIndex||0)-(a.mutantIndex||0);
      case 'videos': return (b.__ch?.videoCount||0)-(a.__ch?.videoCount||0);
      default: return 0;
    }
  });
}
...
/* 동작 중인 API 테스트 함수 등 추가 필요에 따라 구현 */
